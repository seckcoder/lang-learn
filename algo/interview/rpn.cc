...
/* Use class to implement Algebraic Data Type */
class Exp {
  public:
    /* the function to return infix expression */
...
class Num: public Exp {
  public:
    ...
    void parse(const string &line, int &cur) {
      v = 0;
      /* A simple method to parse integer */
...
// The best practice here should be using switch to get
// the priority since modern compiler have corresponding
// optimization for it.
  ...
  public:
    ...
    virtual void unparseSubExp(ostream &out, const Exp *exp) const {
      ...
      if (exp_op && (opCmp(exp_op->op, op) < 0)) {
        // is an operator exp and has smaller priority
        out << "(";
