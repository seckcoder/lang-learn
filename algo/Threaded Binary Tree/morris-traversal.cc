 ...
 * inorder traversal. For traversed nodes, we recover it back to the original binary
 * tree.
 *
 * For every node(current node), if it's left branch is not empty, then it has predecessors.
 * Then we know we should modify the some node(thread node) in the tree so that it right branch
...
vector<int> inorder(TreeNode *p) {
  ...
  while (p) {
    if (!p->left) {
      ...
      TreeNode *pre = p->left; /* pre is the predecessor */
      // we know the rightest node of the tree rooted at p should
