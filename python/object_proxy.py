...
class Proxy(object):
    ...
    # NOTE(by seckcoder) : __getattribute__ will be called for every access,
    # while __getattr__ will be called when the attribute wasn't found the usual
    # ways(that's,when __getattribute__ raised an AttributeError). Therefore,
    # __getattribute__ is used when you want to override calls to existing methods(what does this mean?),
    # __getattr__ is used when you want deal with unusual way to fetching attributes

    # here __local, __args, ... used name mangling(see name_mangling.py)
...
def foo(a, **options):
    print "foo called with ", a, options
    return bar

def bar():
    print "bar"

pobj = PromiseProxy(foo, (123,), { 'name' : 'liwei' })
print pobj.__name__
